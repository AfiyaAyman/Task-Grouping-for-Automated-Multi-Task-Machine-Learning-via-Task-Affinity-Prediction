    ...
    for Type in ['Exponential', 'NonNegative', 'WeightMatrix']:
        ...
        for ClusterType in cluster_algorithms:
            ...
            for count in range(0, len(TASK_Group)):
                ...
                if len(Total_Loss) % 10 == 0:
                    temp_res = pd.DataFrame({'Total_Loss': Total_Loss,
                                             ...
                                             'Individual_RSquare': Individual_RSquare})
                    temp_res.to_csv(
                        f'../Groupwise_Affinity/{datasetName}_MTL_Results_for_{ClusterType}_Clusters_{Type}_{len(Total_Loss)}_run_{run}.csv',
                        index=False)
                    if len(Total_Loss) > 10:
                        old_file = f'../Groupwise_Affinity/{datasetName}_MTL_Results_for_{ClusterType}_Clusters_{Type}_{len(Total_Loss) - 10}_run_{run}.csv'
            ...
            initial_results = pd.DataFrame({'Total_Loss': Total_Loss,
                                            ...
                                            'Individual_Explained_Variance': Individual_Explained_Variance,
                                            'Individual_RSquare': Individual_RSquare})
            initial_results.to_csv(f'{ResultPath}/{datasetName}_MTL_Results_for_{ClusterType}_Clusters_{Type}_run_{run}.csv',
                                   index=False)
