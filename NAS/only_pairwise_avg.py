...
def predictor_NAS(network_architecture, x_train, x_test, y_train, y_test, fold):
    ...
    filepath = f'SavedModels/{dataset_name}_Group_predictor_{fold}_{modelName}.h5'
...
def random_search(col):
    ...
    for iteration in range(1, MAX_EVALS):
        ...
        if length % tail_pointer == 0:
            ...
            results.to_csv(f'{ResultPath}/{dataset_name}_Groupwise_NAS_{length}_{col}_{modelName}.csv',
                           index=False)

            if length > tail_pointer:
                length = length - tail_pointer
                os.remove(
                    os.path.join(f'{ResultPath}/{dataset_name}_Groupwise_NAS_{length}_{col}_{modelName}.csv'))
    ...
    results.to_csv(f'{ResultPath}/{dataset_name}_Groupwise_NAS_{col}_{modelName}_New.csv',
                   index=False)
...
if __name__ == '__main__':
    ...
    DataPath = f'../Results/partition_sample/group_sample/train/'
    ...
    if modelName == 'NN':
        DataPath = f'../Results/partition_sample/group_sample/train/'
        predictor_data = pd.read_csv(f'{DataPath}Groupwise_Features_{dataset_name}_train.csv')
    else:
    ...
    for col in predictor_data.columns:
        print(f'if inf in {col} = {np.isinf(predictor_data[col]).any()}')
    predictor_data.replace([np.inf, -np.inf], np.nan, inplace=True)
    ...
    columns  = list(predictor_data.columns)
    print(columns)
    for col in columns:
        if col != 'pairwise_improvement_average':
            continue
