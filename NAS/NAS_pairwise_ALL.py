...
def predictor_NAS(network_architecture, x_train, x_test, y_train, y_test, fold):
    # dataset_name = 'Landmine'

    filepath = f'SavedModels/{dataset_name}_Pair_predictor_ALL_{fold}_{modelName}.h5'
...
def random_neighbour(current_nn_architecture):
    ...
    if random_hp == 'Features':
        ...
        if len(remaining_features)> 0 and add_remove == 'add':
...
def random_search():
    ...
    for iteration in range(1, MAX_EVALS):
        ...
        if length % tail_pointer == 0:
            ...
            results.to_csv(f'{ResultPath}/{dataset_name}_Pairwise_NAS_{length}_ALL.csv',
                           index=False)

            if length > tail_pointer:
                length = length - tail_pointer
                os.remove(
                    os.path.join(f'{ResultPath}/{dataset_name}_Pairwise_NAS_{length}_ALL.csv'))
    ...
    results.to_csv(f'{ResultPath}/{dataset_name}_Pairwise_NAS_ALL.csv',
                   index=False)
...
if __name__ == '__main__':
    ...
    entire_predictor_data = pd.read_csv(f'{DataPath}Pairwise_Task_Relation_Features_{dataset_name}_train.csv')
    ...
    initial_featureset = np.random.choice(entire_featureSet, 5, replace=False)

    target = entire_predictor_data.pop('Change')
    Sample_Label = np.array(target)
    print(f'entire_featureSet = {entire_featureSet}')
    print(f'initial_featureset = {initial_featureset}')
    # Sample_Inputs = np.array(predictor_data)
    ...
    network_architecture = {'FF_Layers': 3,
                            'FF_Neurons': [100, 50, 30],
                            'learning_rate': 0.001964855475,
                            'activation_function': 'tanh',
                            'output_activation': 'sigmoid',
                            'Features': initial_featureset}

    PROBABILITIES_RANDOM_NEIGHBOUR = [0.1, 0.5, 0.06, 0.05, 0.04, 0.25]
