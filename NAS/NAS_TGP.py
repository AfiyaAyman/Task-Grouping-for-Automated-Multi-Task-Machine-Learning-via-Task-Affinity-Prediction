...
def predictor_NAS(network_architecture, x_train, x_test, y_train, y_test, fold):
    ...
    filepath = f'SavedModels/{dataset_name}_Group_predictor_{fold}_{modelName}_{which_half}.h5'
...
def random_search(col):
    ...
    results.to_csv(f'{ResultPath}/{dataset_name}_Groupwise_NAS_{col}_{modelName}.csv',
                   index=False)
