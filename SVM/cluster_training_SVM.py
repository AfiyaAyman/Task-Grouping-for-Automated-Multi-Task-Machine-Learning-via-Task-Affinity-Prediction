...
def readData_landmine(task_group_list):
    ...
    for landmine in task_group_list:
        ...
        #max_length = 3*min(len(label_0), len(label_1))

        #if len(label_0) > len(label_1):
        #    new_df = [label_0.sample(n=max_length), label_1]

        #    df = pd.concat(new_df)

        # print(len(df))
        max_length_list.append(len(df))
    ...
    return data_param_dictionary,max_length_list
...
def readData_chemical(molecule_list):
    ...
    for molecule in molecule_list:
        ...
        if len(label_0)>100:
            new_df = [label_0.sample(100), label_1.sample(100)]
        else:
            new_df = [label_0, label_1]
        ...
        '''*********************************'''
    print(f'max_length = {max_length_list}, {max(max_length_list)}')
...
def kFold_validation(task_group_list, random_seed, hp_dictionary, dataset):
    ...
    if dataset == 'Landmine':
        data_param_dictionary,max_length_list = readData_landmine(task_group_list)
        print(f'max_length = {max_length_list}, {max(max_length_list)}')
    ...
    print(f'number of samples in each fold: {len(X_list)}')
...
def cluster_Training(dataset, dataframe, best_hp, cluster_algo, cluster_type):
    Task_group = []
    Total_Loss = []
    ...
    Individual_EV = []
    Individual_r_square = []
    ALL_Task_Groups = dataframe['TASK_Group']
    # ALL_Task_Groups = ast.literal_eval(ALL_Task_Groups)



    print(len(ALL_Task_Groups))

    Prev_Groups = {}
    print(f'len tot = {v}, {v+10}')
    for count in range(v,v + 10):
        if count>=len(ALL_Task_Groups):
            break
        task_group = ALL_Task_Groups[count]
    # for task_group in ALL_Task_Groups:
        task_group = ast.literal_eval(task_group)
        # task_group = ALL_Task_Groups[count]
        print(f'task_group = {task_group}, type = {type(task_group)}')
        ...
        if len(Task_group)%10==0:
            print(len(Total_Loss), len(Number_of_Groups), len(Task_group), len(Individual_Group_Score))
            tmp_results = pd.DataFrame({'Total_Loss': Total_Loss,
                                            'Number_of_Groups': Number_of_Groups,
                                            'Task_group': Task_group,
                                            'Individual_Group_Score': Individual_Group_Score,
                                            })
            if dataset == 'Chemical':
                tmp_results['Individual_Error_Rate'] = Individual_Error_Rate
            if dataset == 'Landmine':
                tmp_results['Individual_AUC'] = Individual_AUC
            if dataset == 'Parkinsons':
                tmp_results['Individual_EV'] = Individual_EV
                tmp_results['Individual_r_square'] = Individual_r_square

            tmp_results.to_csv(
                f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{len(Task_group)}_{v}.csv',
                index=False)

            if len(tmp_results)>10:
                os.remove(f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{len(Task_group)-10}_{v}.csv')

    print(len(Total_Loss), len(Number_of_Groups), len(Task_group), len(Individual_Group_Score))
    cluster_results = pd.DataFrame({'Total_Loss': Total_Loss,
                                    'Number_of_Groups': Number_of_Groups,
                                    'Task_group': Task_group,
                                    'Individual_Group_Score': Individual_Group_Score,
                                    })
    if dataset == 'Chemical':
        cluster_results['Individual_Error_Rate'] = Individual_Error_Rate
    if dataset == 'Landmine':
        cluster_results['Individual_AUC'] = Individual_AUC
    if dataset == 'Parkinsons':
        cluster_results['Individual_EV'] = Individual_EV
        cluster_results['Individual_r_square'] = Individual_r_square

    # if v!=0:
    #     df_1 = pd.read_csv(f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{v}_0.csv')
    #
    #     cluster_results = pd.concat([df_1, cluster_results], axis=0)

    cluster_results.to_csv(f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{v}.csv',
                           index=False)
...
if __name__ == "__main__":
    ...
    # cluster_type = 'Exponential'
    cluster_path = f'../Results/Cluster_CSV/{ModelName}/'

    # for cluster_type in ['NonNegative']:
    if cluster_algo == 'Hierarchical':
        clusters_Data = pd.read_csv(f'{cluster_path}{dataset}_Clusters_{cluster_type}_{ModelName}.csv')
    else:
        clusters_Data = pd.read_csv(f'{cluster_path}{dataset}_Clusters_{cluster_type}_{ModelName}_{cluster_algo}.csv')

    cluster_Training(dataset, clusters_Data, hyperparameters_RMTL, cluster_algo, cluster_type)
