...
class TaskGroupingPredictor:

    def __init__(self, dataset_name,counter,run, model_name):
        ...
        if self.model_name == 'SVM':
            self.task_info = pd.read_csv(f'{self.DataPath}Task_Information_{self.dataset_name}_{self.model_name}.csv')
        else:
        ...
        self.single_results = pd.read_csv(f'{self.dataresultPath}/STL/STL_{self.dataset_name}_{self.model_name}.csv')
        self.pair_results = pd.read_csv(
                f'{self.dataresultPath}/Pairwise/{self.model_name}/{self.dataset_name}_Results_from_Pairwise_Training_ALL_{self.model_name}.csv')
    ...
    def predictor_network_evaluate(self, x_train, x_test, y_train, y_test, fold):
        ...
        if self.dataset_name == 'School':
            network_architecture = {'FF_Layers': 2, 'FF_Neurons': [85, 47], 'learning_rate': 0.0013850679916489607,
                                    'activation_function': 'relu', 'output_activation': 'linear',
                                    'Features': ['pairwise_improvement_average', 'pairwise_improvement_variance', 'pairwise_improvement_stddev']}
        if self.dataset_name == 'Chemical':
            network_architecture = {'FF_Layers': 2, 'FF_Neurons': [47, 40], 'learning_rate': 0.0012465611924840647,
        ...
        if self.dataset_name == 'Landmine':
            network_architecture = {'FF_Layers': 9, 'FF_Neurons': [92, 13, 20, 52, 20, 28, 44, 92, 79], 'learning_rate': 0.0015235747908138567,
                                    'activation_function': 'relu', 'output_activation': 'linear',
                                    'Features': ['pairwise_improvement_average', 'number_of_tasks', 'pairwise_improvement_variance', 'group_distance']}
        if self.dataset_name == 'Parkinsons':
            network_architecture = {'FF_Layers': 9, 'FF_Neurons': [92, 13, 20, 52, 20, 28, 44, 92, 79],
                                    'learning_rate': 0.0015235747908138567,
                                    'activation_function': 'relu', 'output_activation': 'linear',
                                    'Features': ['pairwise_improvement_average', 'number_of_tasks',
                                                 'pairwise_improvement_variance', 'group_distance']}
...
#
#     ResultPath = 'Results/new_result/'
#     run = 2
#     new_object = TaskGroupingPredictor(datasetName,0,run)
#     import time
#
#     time_pred_Start = time.time()
#     TaskGroupingPredictor.retrain_predictor(new_object)
