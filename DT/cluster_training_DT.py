...
def cluster_Training(dataset, dataframe, cluster_algo, cluster_type):
    Task_group = []
    Total_Loss = []
    ...
    Prev_Groups = {}
    for count in range(len(ALL_Task_Groups)):
        task_group = ast.literal_eval(ALL_Task_Groups[count])
        ...
        for group_no, task in task_group.items():
            ...
            if grouping_sorted not in Prev_Groups.keys():
                if len(task) == 1:
                    ...
                    if dataset == 'Landmine':
                        loss, task_score, mean_auc, task_auc = kFold_validation(task_group[group_no], random_seed,task_type)
                        # print(f'\n\n\n******{dataset}*****\n')
                        # print(f'loss = {loss}\n'
                        #       f'task_score = {task_score}\n'
                        #       f'AUC = {mean_auc}\n'
                    ...
                    if dataset == 'School':
                        loss, task_score = kFold_validation(task_group[group_no], random_seed, task_type)
                        # print(f'\n\n\n******{dataset}*****\n')
                        # print(f'loss = {loss}\n'
                        #       f'task_score = {task_score}')
        ...
        if len(Task_group)%10==0:
            ...
            tmp_results.to_csv(
                f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{len(Task_group)}_run_{run}.csv',
                index=False)

            if len(tmp_results)>10:
                os.remove(f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_{len(Task_group)-10}_run_{run}.csv')
    ...
    cluster_results.to_csv(f'{cluster_path}/{dataset}_Cluster_Results_{ModelName}_{cluster_type}_{cluster_algo}_run_{run}.csv',
                           index=False)
...
if __name__ == "__main__":
    ...
    for cluster_type in ['Exponential', 'NonNegative']:
        ...
        cluster_Training(dataset, clusters_Data, cluster_algo, cluster_type)
