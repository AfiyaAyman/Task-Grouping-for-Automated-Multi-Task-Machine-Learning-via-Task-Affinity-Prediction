...
def readData(molecule_list):
    ...
    for molecule in molecule_list:
        ...
        # print(f'Sample_Label = {Sample_Label[0]}')
        print(f'shape of Sample_Inputs = {np.shape(Sample_Inputs)}, shape of Sample_Label = {np.shape(Sample_Label)}')
...
def kFold_validation(current_task_specific_architecture, current_shared_architecture, molecule_list, group_no,
    ...
    total_loss_per_task_group_per_fold, error, ap = sort_results(all_scores,molecule_list)

    return total_loss_per_task_group_per_fold, error, ap

def sort_results(all_scores, molecule_list):
...
def final_model(task_hyperparameters, shared_hyperparameters, molecule_list, data_param_dict_for_specific_task,
    ...
    filepath = f'SavedModels/QR_Chemical_Run_{run}_Group_{group_no}_{fold}.h5'
    MTL_model_param = {}
    input_layers = []
...
def mutate_groups(new_task_group, new_group_score):
    ...
    if len(new_task_group[g_old]) == 1:
        ...
        print(f'group deleted and task {task_rand} assigned to group {g_new}')
...
def predictor_network(x_train, y_train):
    ...
    network_architecture = {'FF_Layers': 4, 'FF_Neurons': [14, 10, 21, 15], 'learning_rate': 0.0012465611924840647,
                            'activation_function': 'tanh', 'output_activation': 'linear',
                            'Features': ['pairwise_improvement_average', 'group_stddev', 'pairwise_improvement_variance']}
    ...
    affinity_pred = Model(inputs=Input_FF, outputs=output)
    opt = tf.keras.optimizers.Adam(learning_rate=network_architecture['learning_rate'])
    affinity_pred.compile(optimizer=opt, loss='mse')


    checkpoint = ModelCheckpoint(filepath, verbose=0, monitor='val_loss', save_best_only=True, mode='auto')

    affinity_pred.fit(x=x_train,
                             y=y_train,
                             shuffle=True,
                             epochs=number_of_epoch,
                             batch_size=16,
...
def predict_performance_of_new_group(tasks):
    if len(tasks) > 1:
        ...
        # affinity_pred_arch = {'FF_Layers': 4, 'FF_Neurons': [14, 10, 21, 15], 'learning_rate': 0.0012465611924840647,
        #                       'activation_function': 'tanh', 'output_activation': 'linear',
        #                       'Features': ['pairwise_improvement_average', 'group_stddev',
        #                                    'pairwise_improvement_variance']}

        # affinity_pred_arch = {'FF_Layers': 1, 'FF_Neurons': [63],
        #                       'learning_rate': 0.002027878046573243,
        #                       'activation_function': 'tanh', 'output_activation': 'linear',
        #                       'Features': ['pairwise_improvement_average', 'group_variance']}

        affinity_pred_arch = {'FF_Layers': 2, 'FF_Neurons': [133, 506],
                              'learning_rate': 0.0016620815899787528,
                              'activation_function': 'sigmoid', 'output_activation': 'linear',
                              'Features': ['pairwise_improvement_average', 'number_of_tasks', 'pairwise_Weight_average', 'group_distance', 'pairwise_improvement_stddev', 'pairwise_improvement_variance', 'group_variance', 'group_stddev']}
...
def retrain_predictor(datasetName):
    ...
    affinity_pred_arch = {'FF_Layers': 2, 'FF_Neurons': [133, 506],
                          'learning_rate': 0.0016620815899787528,
                          'activation_function': 'sigmoid', 'output_activation': 'linear',
                          'Features': ['pairwise_improvement_average', 'number_of_tasks', 'pairwise_Weight_average',
                                       'group_distance', 'pairwise_improvement_stddev', 'pairwise_improvement_variance',
                                       'group_variance', 'group_stddev']}
    ...
    predictor_network(x_train, y_train)
...
if __name__ == "__main__":
    ...
    for p in Pairs:
        ...
        Pairwise_err_dict.update({p: 1- pair_res.Avg_Accuracy[0]})
        ap1 = pair_res.Individual_AP_task1[0]
        ap2 = pair_res.Individual_AP_task2[0]
        avg_ap = (ap1 + ap2)/2
        Pairwise_AP_dict.update({p: avg_ap})
    ...
    min_task_groups = 5
    num_folds = 10
    initial_shared_architecture = {'shared_FF_Layers': 1, 'shared_FF_Neurons': [6],
                                   'learning_rate': 0.00029055748415145487}
    # initial_shared_architecture = {'shared_FF_Layers': 1, 'shared_FF_Neurons': [13],
    ...
    '''Predictor Functions'''
    # initial_results = pd.read_csv(f'{ResultPath}/{datasetName}_initial_groups_for_predictor.csv',
    #                               low_memory=False)
    if not rerun:
        ...
        print(f'best results = {np.min(initial_results.Total_Loss)}')
    ...
    for i in range(len(initial_results)):
        group_score = ast.literal_eval(initial_results.Individual_Group_Score[i])
        if not rerun:
            error_rate = ast.literal_eval(initial_results.Individual_Error_Rate[i])
            if 'nan' in initial_results.Individual_AP[i]:
                # print(f'initial_results.Individual_AP[{i}] = {initial_results.Individual_AP[i]}')
                ap = initial_results.Individual_AP[i].replace('nan', '0.85')
                avg_AP = ast.literal_eval(ap)

            else:
                avg_AP = ast.literal_eval(initial_results.Individual_AP[i])
        else:
            error_rate = initial_results.Classification_Error[i]
            avg_AP = initial_results.Average_Precision[i]


        task_group = ast.literal_eval(initial_results.Task_group[i])
        for group_no, score in group_score.items():
            t = tuple(sorted(task_group[group_no]))
            if not rerun:
                Prev_Groups[t] = (group_score[group_no], error_rate[group_no], avg_AP[group_no])
            else:
                Prev_Groups[t] = (group_score[group_no], error_rate, avg_AP)
    ...
    if not rerun:
        ...
        avg_AP = ast.literal_eval(initial_results.Individual_AP[best_group_index])
    else:
        ...
        group_score = ast.literal_eval(initial_results.Individual_Group_Score[best_group_index])
        # error_rate = ast.literal_eval(initial_results.Individual_Error_Rate[best_group_index])
        error_rate = initial_results.Classification_Error[best_group_index]
        # avg_AP = ast.literal_eval(initial_results.Individual_AP[best_group_index])
        avg_AP = initial_results.Average_Precision[best_group_index]
    ...
    if not rerun:
        ...
        Average_Precision.append(np.mean(list(avg_AP.values())))
        ...
        Classification_Error = list(initial_results.Classification_Error)
        Average_Precision = list(initial_results.Average_Precision)

        print(len(actual_improvement_values), len(predicted_improvement_values), len(Switch), len(Iteration_MTL),
              len(Total_Loss))
    ...
    for iteration_MTL in range(init_range, iter_max):
        ...
        # print(f'predicted_performance = {predicted_performance}')
        ...
        if train_model == 1:
                    ...
                    if grouping_sorted not in Prev_Groups.keys():
                        if len(task) == 1:
                            ...
                            if grouping_sorted in Pairwise_res_dict.keys():
                                ...
                                error = Pairwise_err_dict[grouping_sorted]
                                ap = Pairwise_AP_dict[grouping_sorted]
                            else:
                                total_loss_per_task_group_per_fold = Pairwise_res_dict[(grouping_sorted[1], grouping_sorted[0])]
                                error = Pairwise_err_dict[(grouping_sorted[1], grouping_sorted[0])]
                                ap = Pairwise_AP_dict[(grouping_sorted[1], grouping_sorted[0])]

                        else:
                            count += 1

                            total_loss_per_task_group_per_fold, error, ap = kFold_validation(TASK_Specific_Arch[group_no], initial_shared_architecture,
                                                    new_task_group[group_no], group_no, random_seed)

                            print(f'new_task_group[{group_no}] {new_task_group[group_no]}')


                            actual_improvement[group_no] = (tmp_stl_score[
                                                                group_no] - total_loss_per_task_group_per_fold) /tmp_stl_score[group_no]
                        ...
                        Prev_Groups[grouping_sorted] = (total_loss_per_task_group_per_fold, error, ap)
                    else:
                        print(f'new_task_group[{group_no}] = {new_task_group[group_no]}, Prev_Groups[{grouping_sorted}] = {Prev_Groups[grouping_sorted]}')
                        loss, error, ap = Prev_Groups[grouping_sorted]
                        if group_no not in new_group_score.keys():
            ...
            actual_improvement_values.append(actual_improvement.values())
            predicted_improvement_values.append(tmp_predicted_improvement.values())
